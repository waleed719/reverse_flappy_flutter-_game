...
class PlayerProgress extends ChangeNotifier {
  ...
  void addScore(Score score) {
    ...
    if (_topScores.length > maxHighestScoresPerPlayer) {
      _topScores = _topScores.sublist(0, maxHighestScoresPerPlayer);
    }
    notifyListeners();
    final encodedScores = _topScores
        .map((score) => jsonEncode(score.toJson()))
        .toList();
    unawaited(_store.saveTopScores(encodedScores));
  }
}
